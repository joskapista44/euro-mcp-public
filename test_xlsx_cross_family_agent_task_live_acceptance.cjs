...
function summary(result){return {ok:result.ok,outcome:result.outcome,authority:result.authority,noOp:result.noOp,session:result.persistentSession,checks:result.wholeTaskVerification?.checks?.length,readbacks:result.wholeTaskVerification?.readbacks?.length,steps:result.steps?.map(step=>({index:step.index,intent:step.intent,outcome:step.result?.outcome,noOp:step.result?.noOp,retryReceipt:!!step.result?.retryToken}))}}
