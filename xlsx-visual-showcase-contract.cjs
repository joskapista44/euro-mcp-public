...
function buildShowcaseTask(runId){
 ...
 const definedName=`MCP_Sales_${id}`,pivotSourceName=`MCP_PivotSource_${id}`,pivotName=`MCP_Pivot_${id}`,pivotSheet=`MCP_PivotView_${id}`,trendChart=`MCP_Trend_${id}`,regionChart=`MCP_Region_${id}`,trendGeometry=`MCP_TrendSize_${id}`,regionGeometry=`MCP_RegionSize_${id}`,chartWidth=3800000,chartHeight=2300000
 const trendGeometryIdentity=`${trendGeometry}_${chartWidth}x${chartHeight}`,regionGeometryIdentity=`${regionGeometry}_${chartWidth}x${chartHeight}`
 const dataValues=[
