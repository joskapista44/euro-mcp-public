...
function buildShowcaseTask(runId){
 ...
 const definedName=`MCP_Sales_${id}`,pivotName=`MCP_Pivot_${id}`,pivotSheet=`MCP_PivotView_${id}`,trendChart=`MCP_Trend_${id}`,regionChart=`MCP_Region_${id}`
 const dataValues=[
 ...
 const operations=[
  ...
  {intent:'create_pivot',name:pivotName,sourceSheet:data,sourceRange:'A1:E13',rowField:'Region',columnField:'Product',dataField:'Units',styleName:'PivotStyleMedium2',pivotSheet:pivotSheet,destinationRange:'A1',repairIncompletePivot:true,assertions:[{items:['North','Core'],expected:405},{items:['North','Plus'],expected:252},{items:['South','Core'],expected:300},{items:['South','Plus'],expected:200}]},
