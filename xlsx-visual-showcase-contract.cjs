...
function buildShowcaseTask(runId){
 ...
 const definedName=`MCP_Sales_${id}`,pivotName=`MCP_Pivot_${id}`,trendChart=`MCP_Trend_${id}`,regionChart=`MCP_Region_${id}`
 const dataValues=[
 ...
 const operations=[
  {intent:'create_sheet',name:dash},{intent:'write_range',sheet:dash,range:'A1:F16',values:dashValues,formulas:dashFormulas},
  {intent:'create_sheet',name:plan},{intent:'write_range',sheet:plan,range:'A1:E9',values:planValues,formulas:planFormulas},
  {intent:'create_sheet',name:data},{intent:'write_range',sheet:data,range:'A1:H13',values:dataValues,formulas:dataFormulas},
 ...
 return {file_id:null,operations,readbacks:[{sheet:dash,range:'A1:F16'},{sheet:plan,range:'A1:E9'},{sheet:data,range:'A1:H13'}],names:{dash,plan,data,definedName,pivotName,trendChart,regionChart},expected:{totalRevenue:287205,targetRevenue:282000,variance:5205,attainment:287205/282000,operationCount:operations.length,freshWrites:operations.length-1}}
}
