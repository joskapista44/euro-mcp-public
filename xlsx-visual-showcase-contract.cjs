...
function buildShowcaseTask(runId){
 ...
 const definedName=`MCP_Sales_${id}`,pivotSourceName=`MCP_PivotSource_${id}`,pivotName=`MCP_Pivot_${id}`,pivotSheet=`MCP_PivotView_${id}`,trendChart=`MCP_Trend_${id}`,regionChart=`MCP_Region_${id}`
 const dataValues=[
