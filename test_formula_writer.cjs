...
// Community/runtime-compatible path: current live editor exposes GetFormula and SetValue,
// but not the paid-edition-only SetFormula method. Formula strings passed to SetValue must
// still be accepted as formula writes; live same-session read-back remains the real verifier.
const valueCells={}
