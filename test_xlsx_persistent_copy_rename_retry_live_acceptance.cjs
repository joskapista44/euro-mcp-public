...
function secret(id){const v=require('/home/user/marveen/dist/web/vault.js');const r=v.getSecret(id,'xlsx-persistent-copy-rename-retry-live-acceptance');if(!r)throw new Error(`vault secret not found: ${id}`);return r}
...
;(async()=>{
 const tag=String(Date.now()).slice(-7),source=`EURO CRR A ${tag}`,copy=`EURO CRR B ${tag}`,final=`EURO CRR C ${tag}`
