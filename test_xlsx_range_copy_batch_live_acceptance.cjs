...
function secret(id){const v=require('/home/user/marveen/dist/web/vault.js');const r=v.getSecret(id,'xlsx-range-copy-batch-live');if(!r)throw new Error('vault secret not found: '+id);return r}
...
;(async()=>{const tag=String(Date.now()).slice(-7),src='EURO RC SRC '+tag,dst='EURO RC DST '+tag,task={operations:[
...
{intent:'copy_range',sheet:src,range:'A1:B3',targetSheet:dst,targetRange:'D4:E6'}
],readbacks:[{sheet:dst,range:'D4:E6'}]}
const first=await batch.executeBatchTaskInPersistentSession({...options,task});console.log('RANGE COPY BATCH FIRST',JSON.stringify(view(first),null,2));assert.equal(first.ok,true);assert.equal(first.authority,'LIVE_VERIFY');assert.equal(first.noOp,false);assert.equal(first.persistentSession?.oneEditorSession,true);assert.equal(first.persistentSession?.persistenceBarrier?.ok,true);assert.equal(first.wholeTaskVerification?.checks?.every(x=>x.ok),true)
const retry=await batch.executeBatchTaskInPersistentSession({...options,task});console.log('RANGE COPY BATCH RETRY',JSON.stringify(view(retry),null,2));assert.equal(retry.ok,true);assert.equal(retry.authority,'LIVE_VERIFY');assert.equal(retry.noOp,true);assert.equal(retry.persistentSession?.writes,0);assert.equal(retry.persistentSession?.persistenceBarrier,null);assert.equal(retry.wholeTaskVerification?.checks?.every(x=>x.ok),true)
console.log('XLSX RANGE COPY BATCH LIVE ACCEPTANCE: PASS')})().catch(e=>{console.error(e?.stack||e);process.exitCode=1})
