...
async function main(){
 ...
 try{
  ...
  const seed=await frame.evaluate(()=>new Promise(resolve=>window.Asc.editor.callCommand(function(){
    try{Api.GetActiveSheet().GetRange('XFA60:XFC64').SetValue([['Region','Style','Price'],['East','A',10],['West','B',20],['East','B',30],['West','A',40]]);return {ok:true};}
  ...
  steps.push({step:'add-data-field',result:{ok:dataPass,outcome:dataPass?'ok':'verification-failed',source:'live-coedit-editor',operation:'pivot.addDataField',verification:{status:dataPass?'PASS':'FAIL',expected:{dataFields:1},actual:dataActual,readback:'separate-public-inspect'}}});
  ...
  if(pivotSheetName){cleanupActual=await frame.evaluate(sheetName=>new Promise(resolve=>{
    const commandBody=`try{var targetName=${JSON.stringify(sheetName)};var before=Api.GetSheets().map(function(s){return s.GetName();});var target=Api.GetSheet(targetName);if(!target||typeof target.Delete!=='function')return {ok:false,before:before,targetName:targetName,error:'public worksheet Delete target unavailable'};target.Delete();var after=Api.GetSheets().map(function(s){return s.GetName();});return {ok:before.indexOf(targetName)!==-1&&after.indexOf(targetName)===-1,targetName:targetName,before:before,after:after};}catch(e){return {ok:false,targetName:${JSON.stringify(sheetName)},error:String(e&&e.message?e.message:e)}}`;
  ...
  steps.push({step:'cleanup',result:{ok:cleanupPass,outcome:cleanupPass?'ok':'verification-failed',source:'live-coedit-editor',operation:'worksheet.cleanup',verification:{status:cleanupPass?'PASS':'FAIL',expected:{deletedSheet:pivotSheetName,absentAfterDelete:true},actual:cleanupActual,readback:'public-Api.GetSheets'}}});
  const requiredStepNames=['create','inspect-after-create','add-fields','add-data-field','inspect-after-fields','style','rename','inspect-after-rename','refresh','cleanup'];
  const requiredByName=new Map(steps.map(x=>[x.step,x]));
  const requiredSteps=requiredStepNames.map(step=>{const entry=requiredByName.get(step);const status=entry&&entry.result&&entry.result.verification&&entry.result.verification.status;return {step,status:status||'MISSING'};});
  const unexpectedAcceptedSteps=steps.filter(x=>!x.setupOnly&&!requiredStepNames.includes(x.step)).map(x=>x.step);
  const seedStatus=seed&&seed.ok?'PASS':'FAIL';
  const pass=seedStatus==='PASS'&&requiredSteps.every(x=>x.status==='PASS')&&unexpectedAcceptedSteps.length===0;
  console.log(JSON.stringify({milestone:'M6.4',source:'live-coedit-editor',humanObservationRequired:false,steps,outcome:pass?'PASS':'FAIL',verification:{status:pass?'PASS':'FAIL',seedStatus,requiredSteps,unexpectedAcceptedSteps}},null,2));
