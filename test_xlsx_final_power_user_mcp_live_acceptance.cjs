...
function payload(label,r){const p=JSON.parse(r.content[0].text),stepFailed=p.steps?.find(s=>s.result?.ok!==true)?.result,failed=p.failed||stepFailed;console.log(label+' '+JSON.stringify({ok:p.ok,outcome:p.outcome,noOp:p.noOp,session:p.persistentSession||p.session,wholeTaskVerification:p.wholeTaskVerification,diagnostic:p.ok?undefined:{checks:p.checks,failed,stepOutcome:stepFailed?.outcome}},null,2));return p}
;(async()=>{
