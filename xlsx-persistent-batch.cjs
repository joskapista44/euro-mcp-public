...
async function executeBatchTask({task,api}){
 ...
 for(const step of plan.steps){
  ...
  if(step.family==='structural'&&result?.ok&&process.env.EURO_XLSX_STRUCTURAL_BATCH_DEBUG==='1'){
   const op=operations[0],sheet=op.sheet
   if(op.intent==='insert_rows')result.batchDebug={before:result.before,after:result.after,window:result.window,top:await api.readRange({sheet,range:'A1:C1'}),inserted:await api.readRange({sheet,range:'A2:C2'}),bottom:await api.readRange({sheet,range:'A3:C5'})}
  }
