...
The following 768SVZ retry proved that number formatting is not a reliable type witness. The prior currency column passed, but the count columns D and H were returned as strings even with `#,##0` and `General` formats; operation 0 therefore failed closed after one rewritten range, before the defined name, final readbacks or save barrier. The public Spreadsheet API exposes no `ApiRange` type getter, but it does expose `Api.WorksheetFunction.TYPE(ApiRange)`. The live range reader now records that public semantic type code per cell and derives `dataType` from it before falling back to JavaScript getter types. Numeric-string equality is allowed only when `TYPE` proves number identity; a true text `"120"` remains distinct from numeric `120`, independent of display or number format. Static range-reader tests cover both cases. The unchanged 768SVZ final-state retry remains the pending TRUE LIVE gate.


### 768SVZ TYPE runtime regression — OPEN, no acceptance

The fourth user-reported run (`result-768svz-retry3-1337.json`) failed at core operation 0 with 36 mismatches: D/E/H returned the correct numeric strings but `WorksheetFunction.TYPE(ApiRange)` returned 16 for every observed target. The prior claim that this call proved numeric storage identity was premature: only mocked tests and upstream documentation supported it, with no deployed positive control. Code 16 is currently an ambiguous function result, not proof that all these cells contain spreadsheet errors. Do not rerun the 52-operation showcase as a capability probe or replace the check with unconditional Number coercion.

`test_xlsx_numeric_type_live_probe.cjs` collects six existing cell references (D1/E1/H1 and D2/E2/H2), scalar getters, and public TYPE/ISNUMBER/ISTEXT/COUNT/SUM calls alongside numeric, numeric-text and Boolean literal controls. It opens one editor session, makes no mutations, performs no save, closes, and emits ten bounded JSON lines. `typeControlsPass` describes literal controls only; even true does not validate reference conversion. Static tests reproduce both all-16 and literal-pass/reference-16 runtimes. This read-only probe requires the deployed runtime and is the next gate; production numeric type verification remains unaccepted.

Correction to the retry instruction: resuming an incomplete showcase may write the missing defined name, conditional formatting, pivot, charts and panes. Zero writes are required only for a subsequent identical request after the full final state is verified. A save barrier is required when that successful completion performed writes; a zero-write retry has no barrier.

### 768SVZ numeric reference evidence — candidate fix, live gate pending

User's read-only probe at a84e607: literal TYPE controls pass (1/2/4), but numeric D2/E2/H2 references return TYPE=16 and ISNUMBER=false. The same references return COUNT=1 and numeric SUM=120/190/5; header references return COUNT=0 and SUM=0. Scalar numeric text is coerced by COUNT/SUM, so scalar arguments must never be used as cell-type evidence. The underlying TYPE/reference discrepancy remains unexplained.

Production range readback now collects COUNT/SUM from the individual public ApiRange only for stringified numeric constants. Shared verification requires COUNT exactly 1, finite numeric SUM exactly equal to the expected number, matching canonical numeric getter text, and no formula. It no longer relies on TYPE, number format, or a display/raw difference. No extra editor session, writes, or sleeps are introduced. Missing/throwing functions and inconsistent evidence fail closed. Native numeric getters retain their existing path.

Static coverage exercises numeric references, numeric-looking text (COUNT=0), numeric zero versus text zero/blank, wrong values, missing/throwing functions, error results and formulas. Real numeric-looking text reference negative control has not yet been measured in this runtime; mock coverage is not live proof.

`test_xlsx_numeric_readback_live_acceptance.cjs` reads the existing 768SVZ A1:H13 in one session, compares all 36 D/E/H numeric constants against the showcase builder's fixed expectations, checks headers reject numeric expectations, and closes with zero writes. This gate is not full showcase acceptance. After it passes, resume the unchanged 52-operation request; completion can legitimately write the missing objects. Only a subsequent fully completed retry is required to perform zero writes. Defined-name completion, pivot/charts and final readbacks remain pending.

The user-reported read-only numeric gate at commit `285c7ea` passed all 36 fixed numeric constants in D/E/H, rejected the three text headers as numbers, used one editor session, performed zero writes and closed cleanly. This closes the deployed numeric retry regression for the measured showcase data. It does not by itself prove the pending defined name or remaining visual objects.

`test_xlsx_visual_showcase_resume_live_acceptance.cjs` now submits the run-specific 768SVZ 52-operation final-state request through the real MCP. It permits writes needed to complete the prior partial task, requires all 52 read-only whole-task checks, all three same-session readbacks, exact live dashboard KPIs and cross-sheet formula identities, plus the W0.12 session/close/save-barrier contract. A later identical no-op run remains required after completion.


The first full resume after the numeric gate reached the pivot after five verified writes, proving the numeric path and defined-name/conditional-format progress no longer block execution. It failed closed because `InsertPivotNewWorksheet` rejected the showcase's previously unaccepted formula-backed `Revenue` data field; no save barrier or final readbacks ran. Existing TRUE LIVE pivot acceptances all use raw numeric source fields, so the visual task now uses the raw `Units` field over A1:E13 with fixed semantic totals 405/252/300/200. Revenue formulas, KPIs and charts remain unchanged. The precise OnlyOffice rejection mechanism is not claimed from the generic runtime error.

Changing the showcase pivot to the already accepted raw `Units` source did not remove the runtime failure: the next user-reported attempt again failed inside pivot creation with zero writes in that invocation. Therefore formula-backed pivot data was not established as the cause. Before another mutation, `test_xlsx_showcase_pivot_inventory_live_probe.cjs` performs one read-only editor session and reports public worksheet/pivot inventory, source coordinates and field counts. Its purpose is to detect a default-named or partially created pivot/sheet left by an earlier failed insertion; it makes no cleanup decision and claims no pivot acceptance.

The read-only pivot inventory at `7d79efc` found exactly the four expected worksheets, no pivot tables and no orphan pivot sheet. The source worksheet exists with used range A1:H13 and AutoFilter A1:H13. This rules out leftover pivot identity as the cause of repeated insertion failure.

The official ONLYOFFICE `InsertPivotNewWorksheet` example passes a workbook-qualified absolute range returned by `Api.GetRange` (for example `'Sheet1'!$B$1:$D$5`). The production observer previously passed a worksheet-local `ApiWorksheet.GetRange`, whose identity can depend on editor sheet context while the pivot cache is created. Creation now resolves an exactly quoted, workbook-qualified absolute public range and passes that object to `InsertPivotNewWorksheet`; subsequent semantic verification is unchanged. Static regression asserts the exact qualified source and existing pivot/batch/showcase tests pass. Runtime errors now include the exact pivot construction stage. TRUE LIVE validation remains pending.

Workbook-qualified source identity also reached `InsertPivotNewWorksheet` and failed there with zero writes; staged diagnostics prove the failure is inside the API's implicit new-worksheet path. The showcase now pre-creates a deterministic `MCP_PivotView_<run_id>` worksheet through the already accepted co-editing sheet-creation path, then calls the public `InsertPivotExistingWorksheet(sourceRange, destinationRange)` at A1. Planning requires an existing, distinct destination sheet and a single-cell destination; verification additionally binds the pivot parent to that exact sheet. The MCP schema, natural-language assignment and resume gate now carry this destination, making the showcase 53 operations with 53 read-only final checks. The legacy implicit-new-sheet pivot form remains supported for its earlier accepted fixtures.
## 2026-09-18 — Existing-sheet pivot field-order correction

- Live showcase resume reached `InsertPivotExistingWorksheet` successfully, then failed closed at `add-data-field` with `Cannot read properties of null (reading 'length')` after row/column fields had been added.
- Pivot construction now follows the public ONLYOFFICE `AddFields` example order: `AddDataField` first, then `AddFields`. Static coverage fixes this order for the existing-sheet path.
- The showcase explicitly enables recovery of its dedicated pivot sheet. Recovery is allowed only when the incomplete pivot has the exact requested name, source and parent sheet and it is the only pivot on that sheet; all other conflicts remain fail-closed. This safely resumes after the prior partial live mutation without touching shared worksheets.
- Full repository test suite passes. Live co-editing acceptance remains the final gate.

The next live run proved that field order was not causal: `AddDataField` still failed first with zero recorded writes. Source inspection of the public SDK wrapper identified the exact defect: it calls `asc_addDataField`, then dereferences `asc_getDataFields().length` without a null guard. Pivot creation now uses the public equivalent `MoveField(dataField, "Values")`, whose implementation calls `asc_moveToDataField` without constructing an unsafe return wrapper. Static tests make `AddDataField` throw if it is accidentally used and verify the `Values` orientation.

The first `MoveField` live run completed without an exception but produced no persisted pivot; a separate read-only `GetAllPivotTables` inventory confirmed an empty pivot collection. The corrected sequence now establishes row and column fields with `AddFields` before moving the value field with `MoveField(..., "Values")`. This retains the safe wrapper path while giving the existing-sheet pivot a valid layout before the value orientation is applied.

The reordered `MoveField` run also produced no pivot, proving that neither order nor an uncaught exception was causal. The implementation therefore uses the public `AddDataField` mutation again but catches only its measured post-mutation `null.length` return-wrapper defect. It then continues through style, refresh, exact field counts and `GetData` assertions; any missing mutation or unrelated error still fails closed. Static coverage proves both the narrow recovery and rejection of unrelated exceptions.

The narrow `AddDataField` recovery succeeded live and advanced the showcase through the pivot. The next boundary exposed a chart drawing created with the correct name, type and two series but `GetWidth/GetHeight === 0` after the preceding structural operation. Chart creation now detects only this size mismatch and applies public `ApiChart.SetSize`; the existing exact getter verification still rejects any uncorrected size. Static coverage reproduces an `AddChart` implementation that ignores its size arguments and proves the repair.

The live retry proved `SetSize` alone cannot repair geometry while the pivot worksheet remains active. Chart creation now activates and verifies the exact target worksheet through public `SetActive`/`GetActiveSheet` before `AddChart`, then retains the `SetSize` correction and exact dimension readback. Regression coverage models an editor where both initial sizing and `SetSize` depend on active-sheet identity.

Activation also left live geometry at 0x0, while name, type, title and series count were exact. This identifies a command-boundary visibility issue rather than an identity failure. The chart executor now accepts only that exact size-only intermediate state, crosses a fresh `callCommand` boundary in the same editor session, applies public `SetSize`, and requires exact dimension readback before continuing. Any non-size mismatch or failed second readback remains fail-closed; no delay or second browser session is introduced.

The command-boundary chart repair succeeded: both charts and all apply steps completed. Whole-task verification then exposed a two-sheet freeze readback defect: the runtime reports the active worksheet's pane view from an inactive worksheet object. Freeze observation now activates the exact requested worksheet through public `SetActive`, proves it through `GetActiveSheet`, and only then reads `GetFreezePanes`. This context-only read path is allowed during read-only whole verification and enables independent proof of Plan and Data freeze states without content writes or delays.

The next persisted retry reached the already-created pivot with zero writes but failed closed at source identity. A read-only probe proved the persisted pivot, exact parent sheet, one row/column/data field, style and semantic data remain available, while the public `GetSource()` wrapper throws `Cannot read properties of undefined (reading 'ws')` only after reopen. The showcase now creates a second exact workbook name, `MCP_PivotSource_<run_id> = <data>!$A$1:$E$13`, and passes it as `sourceIdentityName`. `GetSource()` remains authoritative when available; only when it is unavailable may the observer use the exact durable name/reference as source proof. Missing, malformed or mismatching markers remain fail-closed. Static coverage proves the reopen fallback and its negative mismatch control. The showcase now has 54 operations/checks; the next live retry may legitimately add this one missing marker before proving the remaining state.

The source marker was accepted live and the retry advanced to the persisted trend chart. Its unique identity, title, two series, presentation, style and anchor position all matched, but `GetWidth/GetHeight` returned 0x0 while another worksheet was active; `SetSize` in that context also remained 0x0. Chart inspection and mutation now activate and prove the exact target worksheet before geometry getters/setters, matching the already required context discipline for creation, deletion and freeze panes. Static regression models reopened inactive-sheet geometry and proves both semantic read and `chart.modify` recover exact dimensions without delays or a second editor session.

The user-reported completion run at `7583fbb` passed all 54 read-only final checks and all three same-session readbacks on `mcp_test.xlsx`: `XLSX VISUAL SHOWCASE RESUME: PASS`. It returned the exact live KPIs 287205 / 282000 / 5205 / 1.0184574468085106, used one editor session, performed two verified chart-geometry repairs, completed one save barrier and closed. The following identical invocation also passed every check but again performed the same two writes. Therefore the visual document and readback contract are accepted, while the mandatory persisted zero-write retry is not yet accepted.

Reopening this deployed runtime can return 0x0 for both named charts even after exact `SetSize` verification and persistence. Each showcase chart now carries an internal exact geometry identity name derived from its canonical width and height. The marker is created only after the complete chart state, including non-zero geometry, has been live-verified; an existing mismatching marker is a hard conflict. On reopen it may substitute for getters only in the measured 0x0 state. Missing markers trigger one final verified repair-and-bind run; the subsequent identical request must prove `noOp=true`, `writes=0` and no persistence barrier. Full repository tests pass; TRUE LIVE zero-write closure remains pending.

The first marker-binding attempt correctly repaired and verified the trend chart to 3800000x2300000, then failed closed because this ONLYOFFICE build rejects a defined name whose `refersTo` is a string constant as `Invalid range`. The marker now encodes the canonical dimensions in its name and uses the chart's exact absolute source range as its valid `refersTo`. This preserves both size and chart-data identity without a hidden cell write. Static coverage requires canonical size-bearing names, valid absolute range references and mismatch rejection.

The valid-range binding run passed with two chart-family writes, but the immediately following identical request still reported one write. Before another mutation, `test_xlsx_showcase_chart_identity_live_probe.cjs` opens one editor session, canonicalizes both showcase chart contracts, and reads each chart plus its exact geometry marker without saving. It reports raw getter dimensions, marker presence/reference/verification and the complete semantic match independently for trend and region. This identifies the remaining family while preserving the workbook unchanged.

The read-only probe found both persisted charts complete except for 0x0 public geometry getters, and found both embedded markers absent. Therefore the single retry write was not evidence that one marker persisted; marker creation inside a chart task was only session-visible and is not a durable supported path. Geometry identities are now two explicit `set_defined_name` batch operations immediately after the two charts. Chart apply accepts exact non-zero public geometry before a marker exists; batch-wide final verification occurs after the explicit marker steps and requires the marker when reopened getters are 0x0. Wrong existing markers remain conflicts. This raises the showcase contract to 56 operations/checks and routes persistence through the already accepted defined-name family.

The live runner now has a strict `EURO_XLSX_REQUIRE_NOOP=1` closure mode. In that mode a successful semantic result is still rejected unless the aggregate result is `noOp=true`, the persistent session reports exactly zero writes, and the persistence barrier is null. The completion invocation remains permissive because it must add the two missing explicit markers; the immediately following strict invocation is the final W0.12 idempotence gate.

### Print/page-layout backlog — OPEN

Power-user print layout is not yet claimed. Add an agent-grade page setup family that can set and live-read print area, orientation, paper size, margins, centering, repeated title rows/columns and fit-to-one-page scaling. Acceptance must prove the requested printable region fits one page without relying on screenshots or fixed delays, and a persisted retry must use zero writes. Until this is implemented and accepted, the MCP can design a compact worksheet but cannot guarantee that printing or PDF export will not split it across pages.


## 2026-09-19 — 768SVZ visual showcase W0.12 zero-write closure — TRUE LIVE PASS

The final strict persisted retry passed on the deployed live runtime for `mcp_test.xlsx` (FILE_ID `1236770`, run `768SVZ`). The invocation used exactly one editor session and returned `noOp=true`, `writes=0`, no persistence barrier, 56/56 read-only whole-task checks and all three same-session final readbacks. Live KPI evidence was total revenue 287205, target revenue 282000, variance 5205 and attainment 1.0184574468085106. Acceptance output: `XLSX VISUAL SHOWCASE ZERO-WRITE RETRY: PASS`.
