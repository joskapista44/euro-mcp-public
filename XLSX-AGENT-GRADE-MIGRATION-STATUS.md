...
Migration proceeds one semantic family at a time. Sort and exact single-field `filter_range` are now agent-grade TRUE LIVE accepted. Filter clearing remains to be migrated. The old direct-Playwright acceptance must not be used as agent-grade authority.
