...
;(async()=>{
 ...
 try{
  ...
  assert.equal((await call({...request,operations:[{intent:'set_chart',sheet:'S',name:'C',range:'A1:B3',chartType:'bar',title:'T',expectedSeriesCount:1}]})).isError,false);assert.equal(calls,2)
  ...
  assert.equal(core.isError,false);assert.equal(calls,3)
