...
function command(spec){
 try{
  ...
  function read(rehydrate){if(rehydrate&&typeof po.initPrintTitles==='function')po.initPrintTitles();return {rows:po.asc_getPrintTitlesHeight(),columns:po.asc_getPrintTitlesWidth()}}
  var before=read(true),actual=spec.axis==='rows'?before.rows:before.columns
  ...
  var state=read(false),now=spec.axis==='rows'?state.rows:state.columns,ok=now===want
