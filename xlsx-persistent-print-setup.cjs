...
function printSetupCommand(spec){
 try{
  ...
  if(spec.mode==='fit_to_pages'){if(typeof ws.setFitToPage!=='function')return {ok:false,outcome:'print-setup-fit-api-unavailable',source:'live-coedit-editor'};ws.setFitToPage(spec.fitToWidth,spec.fitToHeight)}
