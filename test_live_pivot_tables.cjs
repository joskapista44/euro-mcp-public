...
function pivot(name, source) {
  ...
  return {
    state,
    GetName(){return state.name}, SetName(v){state.name=v},
    GetSource(){return {GetAddress(){return state.source}}},
    GetParent(){return {GetName(){return 'PivotSheet'}}},
    ...
    AddFields(spec){
      const rows=spec&&spec.rows!=null?(Array.isArray(spec.rows)?spec.rows:[spec.rows]):[];
      const cols=spec&&spec.columns!=null?(Array.isArray(spec.columns)?spec.columns:[spec.columns]):[];
      state.rows.push(...rows); state.cols.push(...cols); return true;
    },
    AddDataField(f){state.data.push(f);return true}, RefreshTable(){return true}
  };
}
function env(){
  const all=[]; const ranges={};
  global.Api={
    GetAllPivotTables(){return all},
    GetPivotByName(n){const p=all.find(x=>x.GetName()===n);if(!p)throw new Error('missing');return p},
...
ok('create exact serializable readback',()=>{env();const r=run({operation:'pivot.createNewWorksheet',source:'A1:C5',name:'EURO_P'});assert.equal(r.verification.status,'PASS');assert.equal(r.verification.actual.count,1);assert.equal(r.verification.actual.pivot.name,'EURO_P');assert.equal(r.verification.actual.pivot.source,'A1:C5');assert.equal(r.verification.actual.pivot.parentSheet,'PivotSheet')});
...
ok('rename exact readback',()=>{env();run({operation:'pivot.createNewWorksheet',source:'A1:C5',name:'EURO_P'});const r=run({operation:'pivot.rename',name:'EURO_P',newName:'EURO_Q'});assert.equal(r.verification.status,'PASS');assert.equal(r.verification.actual.name,'EURO_Q');assert.equal(r.verification.actual.source,'A1:C5')});
...
ok('refresh fails closed without semantic effect readback',()=>{env();run({operation:'pivot.createNewWorksheet',source:'A1:C5',name:'EURO_P'});const r=run({operation:'pivot.refresh',name:'EURO_P'});assert.equal(r.verification.status,'UNKNOWN');assert.equal(r.ok,false)});
