...
function secret(id){const v=require('/home/user/marveen/dist/web/vault.js');const r=v.getSecret(id,'xlsx-persistent-copy-chain-expanded-retry-live-acceptance');if(!r)throw new Error(`vault secret not found: ${id}`);return r}
...
;(async()=>{
 const tag=String(Date.now()).slice(-7),source=`EURO CCE A ${tag}`,copy=`EURO CCE B ${tag}`,final=`EURO CCE C ${tag}`
 const setup=await executeAgentTaskInPersistentSession({...common,task:{operations:[{intent:'create_sheet',name:source},{intent:'write_range',sheet:source,range:'A1:B2',values:[[11,22],[33,44]]}]}});assert.equal(setup.ok,true);assert.equal(setup.authority,'LIVE_VERIFY')
