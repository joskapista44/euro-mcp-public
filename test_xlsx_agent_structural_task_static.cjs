...
let r=planTask({operations:[{intent:'insert_rows',sheet:'S',range:'A2:A3'}]});assert.equal(r.ok,true);assert.equal(r.operation.type,'rows.insert');assert.equal(r.operation.count,2);assert.equal(r.operation.anchor,2)
r=planTask({operations:[{intent:'delete_columns',sheet:'S',range:'C1:E1'}]});assert.equal(r.ok,true);assert.equal(r.operation.type,'columns.delete');assert.equal(r.operation.count,3);assert.equal(r.operation.anchor,3)
