...
for(const [intent,range] of [['insert_rows','2:3'],['delete_rows','2:2'],['insert_columns','B:C'],['delete_columns','B:B']]){
...
const normalized=batch.planTask({operations:[{intent:'insert_rows',sheet:'S',range:'2:2'}]}).steps[0].operation;assert.equal(normalized.preconditionFingerprint,null);assert.equal(normalized.expectedPostFingerprint,null)
