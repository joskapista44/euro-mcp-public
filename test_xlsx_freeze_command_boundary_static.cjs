...
;(async()=>{
 ...
 calls=[];bodies=[]
 s=session([{ok:false,outcome:'freeze-verification-mismatch-or-unavailable',applied:true,noOp:false,verification:{measurable:true,actual:null,match:false}},{ok:true,outcome:'freeze-observed',applied:false,noOp:false,verification:{measurable:true,actual:null,match:false}}])
 r=await freeze.runCommand(s,op,true)
 assert.equal(r.ok,false);assert.equal(r.outcome,'freeze-verification-mismatch-or-unavailable');assert.equal(r.postMutationObservation.verification.match,false);assert.equal(calls.length,2)
 calls=[];bodies=[]
