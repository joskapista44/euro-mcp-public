...
async function executePivotTask({task,api}){
 ...
 if(!initial?.ok||initial.authority!=='LIVE_READ'||(op.intent!=='delete_pivot_sheet'&&(!identity(initial,op.sourceSheet)||(op.pivotSheet&&!identity(initial,op.pivotSheet)))))return {ok:false,outcome:'xlsx-pivot-task-initial-identity-failed',authority:'PLAN_ONLY',writeAllowed:false,plan}
 ...
 if(!freshInventory?.ok||freshInventory.authority!=='LIVE_READ'||(op.intent!=='delete_pivot_sheet'&&(!identity(freshInventory,op.sourceSheet)||(op.pivotSheet&&!identity(freshInventory,op.pivotSheet)))))return {ok:false,outcome:'xlsx-pivot-task-fresh-boundary-failed',authority:'PLAN_ONLY',writeAllowed:false,plan}
