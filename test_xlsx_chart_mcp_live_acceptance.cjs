...
function payload(r){const p=JSON.parse(r.content[0].text);console.log(JSON.stringify({ok:p.ok,outcome:p.outcome,noOp:p.noOp,callerId:p.callerId,session:p.persistentSession||p.session,wholeTaskVerification:p.wholeTaskVerification},null,2));return p}
;(async()=>{
 ...
 try{
  const suffix=String(Date.now()).slice(-7),sheet=`EURO_MC_${suffix}`,name=`EURO_C_${suffix}`
  const setOps=[{intent:'create_sheet',name:sheet},{intent:'write_range',sheet,range:'A1:B3',values:[['Month','Revenue'],['Jan',10],['Feb',20]]},{intent:'set_chart',sheet,name,range:'A1:B3',chartType:'bar',title:'Revenue',width:3600000,height:2160000,expectedSeriesCount:1}]
  ...
  const delOps=[{intent:'delete_chart',sheet,name}]
