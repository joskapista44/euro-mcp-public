...
const operation=z.object({
 ...
 sourceSheet:str.optional(),sourceRange:str.optional(),sourceIdentityName:str.optional(),rowField:str.optional(),columnField:str.optional(),dataField:str.optional(),styleName:str.optional(),pivotSheet:str.optional(),destinationRange:str.optional(),repairIncompletePivot:z.boolean().optional(),
 chartType:str.optional(),title:str.optional(),expectedSeriesCount:z.number().int().positive().optional(),inRows:z.boolean().optional(),geometryIdentityName:str.optional(),
 ...
 orientation:z.enum(['xlPortrait','xlLandscape']).optional(),topMargin:z.number().nonnegative().optional(),bottomMargin:z.number().nonnegative().optional(),leftMargin:z.number().nonnegative().optional(),rightMargin:z.number().nonnegative().optional(),printGridlines:z.boolean().optional(),printHeadings:z.boolean().optional(),
 fitToWidth:z.number().int().nonnegative().optional(),fitToHeight:z.number().int().nonnegative().optional(),scale:z.number().min(10).max(400).optional(),axis:z.enum(['rows','columns']).optional(),from:z.number().int().positive().optional(),to:z.number().int().positive().optional(),printAreaMode:z.enum(['set','clear','add']).optional(),slot:z.enum(['firstHeader','oddHeader','evenHeader','firstFooter','oddFooter','evenFooter']).optional(),value:z.any().optional(),axis:z.enum(['rows','columns','row','column']).optional(),at:z.number().int().positive().optional()
}).strict().superRefine((op,ctx)=>{
 const fields={insert_rows:['sheet','range'],delete_rows:['sheet','range'],insert_columns:['sheet','range'],delete_columns:['sheet','range'],copy_range:['sheet','range','targetSheet','targetRange'],move_range:['sheet','range','targetSheet','targetRange'],create_sheet:['name'],write_range:['sheet','range','values','formulas'],copy_sheet:['sheet','name'],rename_sheet:['sheet','name'],delete_sheet:['sheet'],move_sheet:['sheet','referenceSheet','position'],clear_range:['sheet','range'],format_range:['sheet','range','format'],layout_range:['sheet','range','type','width','height','hidden'],merge_range:['sheet','range'],unmerge_range:['sheet','range'],freeze_panes:['sheet','mode','range','count'],unfreeze_panes:['sheet'],sort_range:['sheet','range','keyRange','order','hasHeaders'],filter_range:['sheet','range','field','criteria1','criteria2','operator'],clear_filter:['sheet','range'],set_validation:['sheet','range','validationType','alertStyle','operator','formula1','formula2'],clear_validation:['sheet','range'],set_defined_name:['name','refersTo'],rename_defined_name:['name','newName','refersTo'],delete_defined_name:['name'],add_conditional_format:['sheet','range','rule'],delete_conditional_format:['sheet','range','rule'],create_pivot:['name','sourceSheet','sourceRange','sourceIdentityName','rowField','columnField','dataField','styleName','assertions','pivotSheet','destinationRange','repairIncompletePivot'],refresh_pivot:['name','sourceSheet','sourceRange','sourceIdentityName','rowField','columnField','dataField','styleName','assertions'],delete_pivot_sheet:['name','pivotSheet'],set_chart:['sheet','name','range','chartType','title','width','height','expectedSeriesCount','inRows','geometryIdentityName','presentation','chartPosition','series'],rename_chart:['sheet','name','newName','chartType','title','width','height','expectedSeriesCount','presentation','chartPosition','series'],delete_chart:['sheet','name'],set_page_layout:['sheet','orientation','topMargin','bottomMargin','leftMargin','rightMargin','printGridlines','printHeadings'],set_print_setup:['sheet','mode','fitToWidth','fitToHeight','scale','width','height'],set_print_titles:['sheet','axis','from','to'],set_print_area:['sheet','printAreaMode','range'],set_header_footer:['sheet','slot','value'],set_page_break:['sheet','mode','axis','at'],set_first_page_number:['sheet','value']}
 ...
 if(op.intent==='set_first_page_number'&&!(op.value===null||Number.isInteger(op.value)&&op.value>0))ctx.addIssue({code:'custom',path:['value'],message:'First page number must be a positive integer or null'})
 if(op.intent==='set_header_footer'&&!(op.value===null||typeof op.value==='string'))ctx.addIssue({code:'custom',path:['value'],message:'Header/footer value must be string or null'})
})
...
function normalizeOperations(operations){return operations.map(op=>{if(op.intent!=='set_print_area')return op;const normalized={...op,mode:op.printAreaMode};delete normalized.printAreaMode;return normalized})}
...
function makeHandler(deps={}){
 ...
 return async args=>{
  ...
  const {file_id,readbacks}=parsed.data,operations=normalizeOperations(parsed.data.operations),task={operations,readbacks},plan=batch.planTask(task)
...
function register(server,deps){
 server.tool('office_xlsx_batch',
  ...
  'Supports sheet create/copy/rename/delete, range writes plus semantic range copy/move, structural row/column insert/delete, format, fixed layout, merge/unmerge, freeze, sort, filter, durable validation, workbook names, conditional formatting, tested pivot create on new or pre-created worksheet, refresh and delete, named chart set/rename/delete, measurable page layout (orientation, margins, print gridlines/headings), and print setup (fit-to-pages, scale, page size), plus repeating print-title rows/columns and print-area control, plus first/odd/even headers and footers, manual page breaks, and persisted first-page numbering. '+
  ...
  'Core sheet/write operations must precede enhanced operations; all create_sheet operations are executed before write_range so same-batch cross-sheet formulas resolve. AutoFit uses operation-bound retry receipts. '+
...
module.exports={register,makeHandler,schema,normalizeOperations}
