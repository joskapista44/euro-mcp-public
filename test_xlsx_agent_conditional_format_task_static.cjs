...
assert.equal(m.normAddress("'S'!$A$1:$A$3"),'A1:A3')
assert.equal(m.matchRule({type:'xlCellValue',operator:'xlGreater',formula1:'10',fillColor:0xff0000,priority:1,appliesTo:'$A$1:$A$3'},rule),true)
assert.equal(m.matchRule({type:'xlCellValue',operator:'xlGreater',formula1:'11',fillColor:0xff0000,priority:1,appliesTo:'$A$1:$A$3'},rule),false)
function inv(){return {ok:true,authority:'LIVE_READ',sheets:[{name:'S'}]}}
;(async()=>{
 let mutations=0
 const already=await m.executeConditionalFormatTask({task:task(rule),api:{inspect:async()=>inv(),cfObserved:async spec=>spec.type==='cf.inspect'?{ok:true,authority:'LIVE_READ',count:1,rules:[{index:0,type:'xlCellValue',operator:'xlGreater',formula1:'10',fillColor:0xff0000,priority:1,appliesTo:'$A$1:$A$3'}]}:(mutations++,{ok:true,authority:'LIVE_VERIFY'})}})
 ...
 const ambiguous=await m.executeConditionalFormatTask({task:task(rule),api:{inspect:async()=>inv(),cfObserved:async()=>({ok:true,authority:'LIVE_READ',count:2,rules:[{index:0,...rule,fillColor:0xff0000,appliesTo:'A1:A3'},{index:1,...rule,fillColor:0xff0000,appliesTo:'A1:A3'}]})}})
