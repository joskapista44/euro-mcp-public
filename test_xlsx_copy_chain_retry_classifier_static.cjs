...
function cell(address,value){const blank=value===null||value===undefined||value==='';return {address,rawValue:blank?null:value,value:blank?null:value,displayText:blank?'':String(value),formula:null,dataType:blank?'blank':typeof value==='number'?'number':'string'}}
...
;(async()=>{
 ...
 const extendedPlan={ok:true,operations:[{index:0,intent:'copy_sheet',sheet:'Source',name:'Copy'},{index:1,intent:'write_range',sheet:'Copy',range:'C3',values:[[77]],formulas:null},{index:2,intent:'rename_sheet',sheet:'Copy',name:'Final'}]};const extended=await classify([[11,22,null],[33,44,null],[null,null,77]],'A1:C3',extendedPlan);assert.equal(extended.ok,true);assert.equal(extended.authority,'LIVE_VERIFY');assert.equal(extended.retryClassification.targetRange,'A1:C3')
 const dirtyGap=await classify([[11,22,55],[33,44,null],[null,null,77]],'A1:C3',extendedPlan);assert.equal(dirtyGap.ok,false);assert.equal(dirtyGap.outcome,'xlsx-copy-chain-retry-expanded-gap-not-blank');assert.notEqual(dirtyGap.authority,'LIVE_VERIFY')
