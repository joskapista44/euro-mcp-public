...
const normalized=mcp.normalizeOperations([{intent:'set_print_area',sheet:'Report',printAreaMode:'set',range:'A1:H20'}]);assert.deepEqual(normalized,ops);assert.equal(batch.planTask({operations:normalized}).ok,true)
