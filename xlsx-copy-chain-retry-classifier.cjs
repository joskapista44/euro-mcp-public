...
function blankCell(cell){return !normalizeFormula(cell?.formula)&&(cell?.dataType==='blank'||cell?.rawValue===null||cell?.rawValue===undefined||cell?.rawValue==='')&&(cell?.value===null||cell?.value===undefined||cell?.value==='')&&String(cell?.displayText??'')===''}
...
async function classifyCopyChainRetry({plan,api,inventory}){
 ...
 for(let row=expectedTargetUsed.start.row;row<=expectedTargetUsed.end.row;row++)for(let column=expectedTargetUsed.start.column;column<=expectedTargetUsed.end.column;column++){
   ...
   if(!blankCell(targetCell))return {matched:true,ok:false,outcome:'xlsx-copy-chain-retry-expanded-gap-not-blank',authority:'LIVE_READ',writeAllowed:false,row,column}
 }
...
module.exports={parseCell,parseRange,rangeAddress,unionRange,finalName,writesForCopy,blankCell,classifyCopyChainRetry}
