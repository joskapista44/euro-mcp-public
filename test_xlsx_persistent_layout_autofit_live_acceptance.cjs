...
;(async()=>{const tag=String(Date.now()).slice(-7),sheet=`EURO AF ${tag}`;const setup=await executeAgentTaskInPersistentSession({...common,task:{operations:[{intent:'create_sheet',name:sheet},{intent:'write_range',sheet,range:'A1:C3',values:[['A very long heading that should force a wider column','x','y'],['another long value for autofit verification','short','short'],['third long value','z','z']]},{intent:'format_range',sheet,range:'A1:C3',format:{fontSize:18}}]}});assert.equal(setup.ok,true)
const ops=[{range:'A1:A3',type:'columns.autofit'},{range:'1:3',type:'rows.autofit'}]
