...
async function preflightReferencedSheets(frame,apiHely,sheets,callbackTimeoutMs=15000){
  return frame.evaluate(({u,names,timeout})=>new Promise(resolve=>{
    ...
    try{editor.callCommand(new Function(body),false,value=>finish(value===undefined?{ok:false,outcome:'empty-callback',source:'live-coedit-editor',error:'sheet preflight callback returned undefined'}:value))}catch(err){finish({ok:false,outcome:'callcommand-error',source:'live-coedit-editor',error:String(err&&err.message?err.message:err)})}
