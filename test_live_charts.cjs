...
async function main(){
 ...
 {
  const {api,charts}=makeApi({directDelete:false}); global.Api=api
  chartCommand({type:'chart.create',sheet:'Sheet1',range:'A1:B3',chartType:'bar',name:'M51_FRAME_DELETE',title:'Revenue',width:3600000,height:2160000})
  let focused=false
  const page={keyboard:{press:async key=>{assert.equal(key,'Delete');const selected=charts.find(c=>c._selected());assert.ok(selected,'Delete key must follow public Select');assert.equal(selected._deleteSelected(),true)}}}
  const frame={
    page:()=>page,
    evaluate:async(fn,arg)=>{
      if(arg===undefined){global.window={focus:()=>{focused=true}};return fn()}
      global.window={Asc:{editor:{callCommand:(cmd,_recalc,cb)=>cb(cmd())}}}
      global.Api=api
      return fn(arg)
    }
  }
  const r=await runChartInFrame(frame,'window.Asc.editor',{type:'chart.delete',sheet:'Sheet1',name:'M51_FRAME_DELETE'},1000)
